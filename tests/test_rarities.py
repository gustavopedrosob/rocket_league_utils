import pytest
from test_items import sample_items
from rl_data_utils.exceptions import RarityNotExists


sample_items = sample_items.get_items_with_valid_rarity()


def test_get_items_by_rarity_rarity_not_exists():
    with pytest.raises(RarityNotExists):
        sample_items.get_items_by_rarity('')


def test_get_items_by_rarity():
    print(sample_items.get_items_by_rarity("Very rare").items)


def test_get_items_by_rarity_regex():
    print(sample_items.get_items_by_rarity_regex('v').items)


def test_get_items_by_rarity_equal_to():
    print(sample_items.get_items_by_rarity_equal_to("Very rare").items)


def test_get_items_by_rarity_contains():
    print(sample_items.get_items_by_rarity_contains("V").items)


def test_get_rarities():
    print(sample_items.get_rarities())


def test_get_items_common():
    print(sample_items.get_items_common().items)


def test_get_items_legacy():
    print(sample_items.get_items_legacy().items)


def test_get_items_uncommon():
    print(sample_items.get_items_uncommon().items)


def test_get_items_rare():
    print(sample_items.get_items_rare().items)


def test_get_items_very_rare():
    print(sample_items.get_items_very_rare().items)


def test_get_items_import():
    print(sample_items.get_items_import().items)


def test_get_items_exotic():
    print(sample_items.get_items_exotic().items)


def test_get_items_black_market():
    print(sample_items.get_items_black_market().items)


def test_get_items_limited():
    print(sample_items.get_items_limited().items)
