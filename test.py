...
import pyperclip
from rl_data_utils.decorators import rl_data_smart_attributes, validate_attributes


@rl_data_smart_attributes
def test(certified, type_, color, rarity):
    print(certified, type_, color, rarity)


init = datetime.now()
test(certified='ainsiansinsainsa', type_='bodies', color='tw', rarity='vr')
